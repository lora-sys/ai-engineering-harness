...
把 projectRoot 校验整段注释掉，套件依然全绿 —— 一条测试如果只断言"退出码非零"，
那么任何原因造成的非零都算通过，包括一次无关的崩溃。更糟的是：有一条**已发布**的
断言与 secret 检测器的设计直接矛盾（fixture 把密钥放在 `src/config.ts`，而检测器
恰好整个跳过这个路径），也就是说它**从写下的那天起就不可能通过**。修法是引入会打印
"实际看到了什么"的 `assert_has` / `assert_lacks` / `assert_eq`。

> **⚠️ 这一节的一次自我更正。** 这里原本写着「bats 1.13 只有最后一条命令的退出码算数，
> 中途裸 `[[ ]]` 会被静默吞掉，所以本地过、CI（bats 1.10）红」。**那是错的，而且是我写的。**
> 实测四种组合（bash 3.2.57 / 5.3.15 × bats 1.10.0 / 1.13.0），一条保证为假的
> 中途 `[[ ]]` **在四种组合下全都如实失败**。「本地过、CI 红」的真实原因平淡得多：
> 那次本地跑根本没把 dashboard 那个文件跑进去。拿修复前的代码单跑那一条，本地复现
> 得和 CI 一模一样，同一行 376。留着这段更正，是因为一份讲"别信没验证的说法"的
> 文档，自己带了一条没验证的说法。
